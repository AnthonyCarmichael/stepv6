#ifndef ST_OPENACTIVECONTOURS_HPP
#define ST_OPENACTIVECONTOURS_HPP
...
ST_OpenActiveContours<DataT>::ST_OpenActiveContours(ConstHoughSpacePtrConst hough_space,
...
{
    Vec4d head_point = _hough_space->pixel_to_cartesian( start_pixel );

    // Calcule la bbox correspondant a un cube centre autour de l'extremite du snake et de taille coneSizePix
    Vec4i search_cone_size_vec( 0, search_cone_size, search_cone_size, search_cone_size);
    Vec4i cone_bbox_bot = start_pixel - search_cone_size_vec;
    Vec4i cone_bbox_top = start_pixel + search_cone_size_vec;
    Vec4i hs_dim = _hough_space->dim();
    ...
    for( int axe = 0 ; axe < 4 ; axe++ )
    {
        ...
        if( cone_bbox_bot[axe] >= hs_dim[axe] )
        {
            cone_bbox_bot[axe] = hs_dim[axe] - 1;
        }
        ...
        if( cone_bbox_top[axe] >= hs_dim[axe] )
        {
            cone_bbox_top[axe] = hs_dim[axe] - 1;
        }
    }
    ...
    std::vector< DirScorePair > potential_grow_dirs_and_scores;
    ...
    if( v1.norm() == 0 )
    {
        PS_LOG->addErrorMessage(LogInterface::error, QObject::tr("ST_OpenActiveContours constructor: after PCA, direction has a norm of 0! %1 %2 %3 %4").arg(v1[0]).arg(v1[1]).arg(v1[2]).arg(v1[3]));
        exit( EXIT_FAILURE );
    }
    v1 *= _hough_space->wres();
...
template< class DataT >
void ST_OpenActiveContours<DataT>::get_dir_and_score_inside_bbox(const Vec4i& center_pixel,
...
{
    Vec4i search_bbox_size_vec( search_bbox_size_radius, search_bbox_size_spatial, search_bbox_size_spatial, search_bbox_size_spatial);
    Vec4i bbox_bot = center_pixel - search_bbox_size_vec;
    Vec4i bbox_top = center_pixel + search_bbox_size_vec;

    // Il ne reste plus qu'a regarder dans la boite pour faire une acp des directions ponderees
    // On parcour donc toute la bbox et on recupere les directions de croissance potentielles
    // Avec leur scores dans l'espace de Hough
    Vec4i curr_pixel_in_bbox;
    for ( curr_pixel_in_bbox[0] = bbox_bot[0] ; curr_pixel_in_bbox[0] <= bbox_top[0] ; curr_pixel_in_bbox[0]++ )
    {
        for ( curr_pixel_in_bbox[1] = bbox_bot[1]; curr_pixel_in_bbox[1] <= bbox_top[1] ; curr_pixel_in_bbox[1]++ )
        {
            ...
            {
                for ( curr_pixel_in_bbox[3] = bbox_bot[3] ; curr_pixel_in_bbox[3] <= bbox_top[3]; curr_pixel_in_bbox[3]++ )
                {
                    DataT cur_pixel_value = _hough_space->value(curr_pixel_in_bbox[0], curr_pixel_in_bbox[1], curr_pixel_in_bbox[2], curr_pixel_in_bbox[3]);

                    if( cur_pixel_value > min_value_to_consider && center_pixel != curr_pixel_in_bbox )
                    {
                        Vec4d curr_point_in_bbox = _hough_space->pixel_to_cartesian(curr_pixel_in_bbox);

                        // Calcul de la direction potentielle de croissance
                        Vec4d  potential_grow_dir = curr_point_in_bbox - reference_point;
                        double potential_grow_dir_norm_3d = potential_grow_dir.tail(3).norm();

                        Vec4d weighted_direction = potential_grow_dir * static_cast<double>(cur_pixel_value) * ((static_cast<double>(search_bbox_size_spatial) * _hough_space->xres() ) - potential_grow_dir_norm_3d);
    ...
    if( sort_crescent_order )
    {
        ...
        std::sort(out_dir_and_scores.begin(),
                  ...
                  [](const DirScorePair& pair1, const DirScorePair& pair2) -> bool{ return pair1.second < pair2.second ; } );
    }
...
template< class DataT >
void ST_OpenActiveContours<DataT>::get_dir_and_score_inside_cone(const Vec4i& center_pixel,
                                                                 const Vec4d& reference_point,
                                                                 ...
                                                                 std::vector< DirScorePair >& out_dir_and_scores) const
{
    ...
    Vec4i bbox_bot = center_pixel - search_bbox_size_vec;
    Vec4i bbox_top = center_pixel + search_bbox_size_vec;
    Vec3d cone_direction_spatial = cone_direction.tail(3).normalized();
            ...
            for ( curr_pixel_in_bbox[2] = bbox_bot[2] ; curr_pixel_in_bbox[2] <= bbox_top[2] ; curr_pixel_in_bbox[2]++ )
            {
                for ( curr_pixel_in_bbox[3] = bbox_bot[3] ; curr_pixel_in_bbox[3] <= bbox_top[3]; curr_pixel_in_bbox[3]++ )
                {
                    ...
                    if( cur_pixel_value > min_value_to_consider && center_pixel != curr_pixel_in_bbox )
                    {
                        ...
                        // Calcul de la direction potentielle de croissance
                        Vec4d  potential_grow_dir         = curr_point_in_bbox - reference_point;
                        Vec3d  potential_grow_dir_spatial = potential_grow_dir.tail(3);
...
template< class DataT >
std::vector< CT_Circle* > ST_OpenActiveContours<DataT>::get_raw_circles() const
{
    std::vector< CirclePtr > rslt;
...
template< class DataT >
void ST_OpenActiveContours<DataT>::append_raw_circles_to_vector( std::vector< CT_Circle* >& in_out_circle_vector ) const
{
    int n_points = get_n_points();

    for( int i = 0 ; i < n_points ; i++ )
    {
        const Vec4d& point = _points.row(i);
        in_out_circle_vector.push_back( new Circle( new CT_CircleData(point.tail(3), Vec3d::UnitZ(), point[0]) ) );
    }
}

template< class DataT >
void ST_OpenActiveContours<DataT>::markRepulsion( double repulse_factor )
{
    ...
    for( int i = 0 ; i < n_points ; i++ )
    {
        const Vec4d& curr_point = _points.row(i);
        ...
        Vec4i currPix = _hough_space->cartesian_to_pixel(curr_point);
        ...
        for ( int currW = 0 ; currW < dimw ; currW++ )
        {
            ...
            double r_elt = _hough_space->pixelToCartesianW( currW );
            ...
            Vec4i bot = _hough_space->cartesian_to_pixel(r_elt,
                                                         curr_point[1] - sum_radius_times_repulse_factor,
                                                         curr_point[2] - sum_radius_times_repulse_factor,
                                                         curr_point[3] - sum_radius_times_repulse_factor);
            Vec4i top = _hough_space->cartesian_to_pixel(r_elt,
                                                         curr_point[1] + sum_radius_times_repulse_factor,
                                                         curr_point[2] + sum_radius_times_repulse_factor,
                                                         curr_point[3] + sum_radius_times_repulse_factor);
            ...
            for( int axe = 0 ; axe < 4 ; axe++ )
            {
                ...
                if( bot[axe] >= hs_dim[axe] )
                {
                    bot[axe] = hs_dim[axe] - 1;
                }
                if( top[axe] < 0 )
                {
                    top[axe] = 0;
                }
                if( top[axe] >= hs_dim[axe] )
                {
                    top[axe] = hs_dim[axe] - 1;
                }
            }
...
template< class DataT >
void ST_OpenActiveContours<DataT>::resample( double sampleRes )
{
    ...
    while( nextPointLength < totalLength && nextPointIndex < noldpts )
    {
        ...
        while( currLength >= nextPointLength )
        {
            ...
            if( indexNewPoint < _points.rows() )
            {
                ...
                Vec4d segmentDir = curr_point - next_point;
...
template< class DataT >
double ST_OpenActiveContours<DataT>::length3D() const
{
    double length = 0;
    int n_points = get_n_points();
...
template< class DataT >
void ST_OpenActiveContours< DataT >::grow(int nIterMax,
                                          double growCoeff,
                                          ...
                                          double seuilSigmaL1)
{
    ...
    for( i = 0 ; i < nIterMax && ( sigmaL1Head > seuilSigmaL1 || sigmaL1Back > seuilSigmaL1 ) && ( hasGrownBack || hasGrownHead ) ; i++ )
    {
        ...
        {
            Vec4d head = _points.row(0);
            Vec4d new_head = head  - ( growCoeff * growDirHead );
            Vec4d tail = _points.row(get_n_points()-1);
            Vec4d new_tail = tail - ( growCoeff * growDirBack );

            Eigen::MatrixXd new_points( get_n_points() + 2, 4 );
            new_points.row(0)                           = new_head;
            new_points.row(get_n_points()+1)            = new_tail;
            new_points.block(1, 0, get_n_points(), 4)   = _points.block(0, 0, get_n_points(), 4);
        ...
        else if (hasGrownHead)
        {
            ...
            Eigen::MatrixXd new_points( get_n_points() + 1, 4 );
            new_points.row(0)                           = new_head;
            new_points.block(1, 0, get_n_points(), 4)   = _points.block(0, 0, get_n_points(), 4);
        ...
        else if (hasGrownBack)
        {
            ...
            new_points.block(0, 0, get_n_points(), 4)   = _points.block(0, 0, get_n_points(), 4);
...
template< class DataT >
void ST_OpenActiveContours< DataT >::getGrowingDirections(double coneAngleDegres,
                                                          int coneSizePixels,
                                                          double seuilSigmaL1,
                                                          ...
                                                          Vec4d& outGrowDirBack ) const
{
    ...
    if( outSigmaL1Head > seuilSigmaL1 && outHasGrownHead )
    {
        ...
        Vec4d head_point = _points.row(0);
        Vec4i head_pixel = _hough_space->cartesian_to_pixel(head_point[0], head_point[1], head_point[2], head_point[3]);
        Vec4d head_tangent = getTangentAtHead(true);
        head_tangent *= -1;
    ...
    if( outSigmaL1Back > seuilSigmaL1 && outHasGrownBack )
    {
        ...
        Vec4d tail_point   = _points.row( get_n_points() - 1 );
        Vec4i tail_pix     = _hough_space->cartesian_to_pixel(tail_point[0], tail_point[1], tail_point[2], tail_point[3]);
        Vec4d tail_tangent = getTangentAtTail(true);
...
template< class DataT >
Eigen::Vector4d ST_OpenActiveContours< DataT >::getGradientEnergyGrowProportionalResolution(const Vec4d& snakeExtremityPoint,
...
{
    // Calcule la bbox correspondant a un cube centre autour de l'extremite du snake et de taille coneSizePix
    Vec4i coneBBoxBot = snakeExtremityPix - Vec4i( 5, coneSizePixels, coneSizePixels, coneSizePixels);
    Vec4i coneBBoxTop = snakeExtremityPix + Vec4i( 5, coneSizePixels, coneSizePixels, coneSizePixels);
    Vec4i hs_dim = _hough_space->dim();
    ...
    for( int axe = 0 ; axe < 4 ; axe++ )
    {
        ...
        if( coneBBoxBot[axe] >= hs_dim[axe] )
        {
            coneBBoxBot[axe] = hs_dim[axe] - 1;
        }
        if( coneBBoxTop[axe] < 0 )
        {
            coneBBoxTop[axe] = 0;
        }
        if( coneBBoxTop[axe] >= hs_dim[axe] )
        {
            coneBBoxTop[axe] = hs_dim[axe] - 1;
        }
    }
    ...
    Vec3d v1_normalized_3d = v1.tail(3).normalized();

    Vec3d tangent_3d = snakeTangentAtExtremity.tail(3);
    ...
    Vec4d grow_dir = v1 * fabs( scalr_prod );
...
template< class DataT >
Eigen::Vector4d ST_OpenActiveContours< DataT >::getTangentAtPoint( int i, bool normalize ) const
{
    Vec4d rslt;
    Vec4d curr = _points.row(i);

    if( i == 0 )
    {
        // Tangente par difference finie
        Vec4d next = _points.row( i+1 );
        rslt = next - curr;
    }
    else if( i == get_n_points() - 1 )
    {
        // Tangente par difference finie
        Vec4d prev = _points.row( i-1 );
        rslt = curr - prev;
    }
    else
    {
...
template< class DataT >
void ST_OpenActiveContours< DataT >::updatePoints()
{
    // On regarde le statut de chaque point
    int n_pts = get_n_points();
    Vec4d bbox_bot = _hough_space->boundingBoxBot();
    Vec4d bbox_top = _hough_space->boundingBoxTop();
...
template< class DataT >
void ST_OpenActiveContours< DataT >::getGeometricMatrix(double alpha, double beta, Eigen::MatrixXd& outGeometricMatrix) const
{
    ...
    for( int i = 0 ; i < npts ; i++ )
    {
        if( i >= 2 && i < npts-2 )
        {
            ...
            if( i-1 >= 0 )
            {
                outGeometricMatrix(i,i-1) = -alpha - 4*beta;
            }
...
template< class DataT >
void ST_OpenActiveContours< DataT >::getAPlusRhoIInverse(double alpha, double beta, double timeStep, Eigen::MatrixXd& outAPlusRhoIInverse ) const
{
    // Calcul de A+rho*I
    int npts    = get_n_points();
...
template< class DataT >
void ST_OpenActiveContours< DataT >::updatePointsAndGetAverageAndStDevMovement(const Eigen::MatrixXd &newPoints,
                                                                               ...
                                                                               double& outStdDevMovement )
{
    ...
    for( int i = 0 ; i < npts ; i++ )
    {
        ...
        _hough_space->cartesian_to_pixel(curPoint[0], curPoint[1], curPoint[2], curPoint[3]);

        movement = (Vec4d(_points.row(i)) - curPoint).norm();
        movementSum += movement;
        movementSquareSum += movement*movement;

        if( _hough_space->isPixelIn(curPix) )
        {
...
template< class DataT >
void ST_OpenActiveContours< DataT >::relax(int nIterMax,
                                           ...
                                           double threshAverageMovement4D )
{
    ...
    double rho = 1.0/timeStep;
    Eigen::MatrixXd pointsNextIteration( npts, 4 );

    double averageMovement4D = std::numeric_limits<double>::max();
    double stDevMovement4D = std::numeric_limits<double>::max();
    ...
    for( relaxStep = 1 ; relaxStep < iNIterMax && averageMovement4D > threshAverageMovement4D ; relaxStep++ )
    {
        ...
        QFuture<Eigen::MatrixXd*> futureGradientEnergyImage = QtConcurrent::run( this, &ST_OpenActiveContours< DataT >::getGradientEnergyOrthogonalImageMatrixMultipliedByTangentNorm, globalWeight );
        QFuture<Eigen::MatrixXd*> futureSecondDiffOnTangentDivByTanNorm = QtConcurrent::run( this, &ST_OpenActiveContours< DataT >::getSecondDifferentialOnTangentDividedByTangentNormMultiplyedByImageEnergy, globalWeight );
...
template< class DataT >
Eigen::MatrixXd* ST_OpenActiveContours< DataT >::getGradientEnergyOrthogonalImageMatrixMultipliedByTangentNorm( double globalWeight ) const
{
    int npts = get_n_points();
...
template< class DataT >
Eigen::MatrixXd* ST_OpenActiveContours< DataT >::getGradientEnergyOrthogonalImageMatrix( double globalWeight ) const
{
    int npts = get_n_points();
    Eigen::MatrixXd* rslt = new Eigen::MatrixXd( npts, 4 );
    ...
    for( int i = 0 ; i < npts ; i++ )
    {
        QFuture<Vec4d> futureCurrGrad = QtConcurrent::run( this, &ST_OpenActiveContours< DataT >::gradientEnergyImageInterpolatedAtPoint, i, globalWeight );
        QFuture<Vec4d> futureDirectionContours = QtConcurrent::run( this, &ST_OpenActiveContours< DataT >::directionContoursAtPoint, i );

        futureCurrGrad.waitForFinished();
        futureDirectionContours.waitForFinished();

        Vec4d currGrad = futureCurrGrad.result();
        Vec4d directionContour = futureDirectionContours.result().normalized();
...
template< class DataT >
Eigen::Vector4d ST_OpenActiveContours< DataT >::gradientEnergyImageInterpolatedAtPoint(int indexPoint, double globalWeight) const
{
    Vec4d currPoint         = _points.row(indexPoint);                                                                  // ieme point du contour
    Vec4i currPixel         = _hough_space->cartesian_to_pixel(currPoint[0], currPoint[1], currPoint[2], currPoint[3]); // Pixel contenant le ieme point
    Vec4d currPixelCenter   = _hough_space->pixel_to_cartesian(currPixel[0], currPixel[1], currPixel[2], currPixel[3]); // Centre en cartesien du pixel contenant le ieme point
    ...
    for( int i = 0 ; i < 4 ; i++ )
    {
        ...
        {
            Vec4i prevPixel         = currPixel; prevPixel[i] -= 1;
            Vec4d prevPixelPoint    = _hough_space->pixel_to_cartesian(prevPixel[0], prevPixel[1], prevPixel[2], prevPixel[3]);
        ...
        {
            Vec4i nextPixel         = currPixel;  nextPixel[i] += 1;
            Vec4d prevPixelPoint    = _hough_space->pixel_to_cartesian(nextPixel[0], nextPixel[1], nextPixel[2], nextPixel[3]);
...
template< class DataT >
double ST_OpenActiveContours< DataT >::partialDerivativeEnergyImageAtPixelInImage(const Vec4i &p, double globalWeight, int coordToDerive) const
{
    ...
    Vec4i prevInDirection = p; prevInDirection[coordToDerive] -= 1;
    bool isPrevInImage = _hough_space->isPixelIn(prevInDirection);

    // On recupere le pixel suivant selon cette variable
    Vec4i nextInDirection = p; nextInDirection( coordToDerive ) += 1;
    bool isNextInImage = _hough_space->isPixelIn(nextInDirection);
...
template< class DataT >
double ST_OpenActiveContours< DataT >::partialDerivativeEnergyImageAtPixelOutsideImage(const Vec4i& p, double globalWeight, int coordToDerive) const
{
    Vec4i hough_space_dim = _hough_space->dim();
...
template< class DataT >
double ST_OpenActiveContours< DataT >::energyImageLocalAtPixel(const Vec4i& p) const
{
    DataT min, max;

    // Recupere les min et max du voisinage
    _hough_space->getMinMaxInNeighbourhood(p, min, max, 1);
    ...
    return static_cast<double>( min - _hough_space->valueHough(p[0], p[1], p[2], p[3]) ) / static_cast<double>( max - min );
}

template< class DataT >
Eigen::MatrixXd* ST_OpenActiveContours< DataT >::getSecondDifferentialOnTangentDividedByTangentNormMultiplyedByImageEnergy( double globalWeight ) const
{
    int npts = get_n_points();
    ...
    for( int i = 0 ; i < npts; i++ )
    {
        ...
        for( int c = 0 ; c < 4 ; c++ )
        {
            Vec4i pixel_at_i = _hough_space->cartesian_to_pixel(point[0], point[1], point[2], point[3]);
...
template< class DataT >
Eigen::Vector4d ST_OpenActiveContours< DataT >::directionContoursAtPoint(int indexPoint) const
{
    Vec4d rslt;
                   ...
                   double& out_l1, double& out_l2, double& out_l3, double& out_l4)
{
    ...
    if( dir_score_pairs.size() < 4 )
    {
        PS_LOG->addErrorMessage(LogInterface::error, QObject::tr("ST_OpenActiveContours::uncenteredPCA: not enough directions for pca"));
        exit( EXIT_FAILURE );
    }

    int n_dirs = dir_score_pairs.size();

    // On cree la matrice 4*n_dirs et on la remplit (conversion du tableau de points en matrice en fait)
    Eigen::MatrixXd dir_matrix( 4, n_dirs );
    ...
    Eigen::Matrix4d cov = dir_matrix * dir_matrix.transpose();
    ...
    Eigen::Vector4d eigenValues = eigenSolver.eigenvalues().real();
    Eigen::Matrix4d eigenVectors = eigenSolver.eigenvectors().real();  // Les colonnes de cette matrice sont les vecteurs propres associes aux valeurs propres
    ...
    out_v4 << eigenVectors(0,3), eigenVectors(1,3), eigenVectors(2,3), eigenVectors(3,3);
}

#endif // ST_OPENACTIVECONTOURS_HPP
